...
const budgetRanges = [
  {
    ...
    min: 0,
    max: 300,
    path: '/catalog?minPrice=0&maxPrice=300',
    color: 'from-green-500/10 to-green-600/10 border-green-500/30 text-green-700'
  },
  {
    label: '300 - 600',
    min: 300,
    max: 600,
    path: '/catalog?minPrice=300&maxPrice=600',
  ...
  },
  {
    ...
    min: 600,
    max: 1000,
    path: '/catalog?minPrice=600&maxPrice=1000',
    color: 'from-site-accent/10 via-site-accent/5 to-slate-teal/10 border-site-accent/30 text-site-accent'
  },
  {
    label: '1,000 - 2,000',
    min: 1000,
    max: 2000,
    path: '/catalog?minPrice=1000&maxPrice=2000',
    color: 'from-orange-500/10 to-orange-600/10 border-orange-500/30 text-orange-700'
  },
  {
    label: '2,000+',
    min: 2000,
    max: 10000,
    path: '/catalog?minPrice=2000&maxPrice=10000',
    color: 'from-site-primary/10 to-slate-teal/10 border-site-primary/30 text-site-primary'
  }
];

export default function BudgetMegaMenu({ onClose }: BudgetMegaMenuProps) {
  const [minPrice, setMinPrice] = useState(0);
  const [maxPrice, setMaxPrice] = useState(5000);

  const handleCustomSearch = () => {
    window.location.href = `/catalog?minPrice=${minPrice}&maxPrice=${maxPrice}`;
    onClose();
  };
    ...
    <div className="w-full bg-white/95 backdrop-blur-xl shadow-2xl border-t border-soft-grey/20">
      <div className="max-w-[1600px] mx-auto px-6 py-8">
        ...
        <div className="mb-8 p-6 rounded-xl bg-gradient-to-br from-site-accent/5 via-off-white/50 to-slate-teal/5 border border-soft-grey/30 shadow-sm">
          <h4 className="text-sm font-semibold text-site-primary mb-4">Custom Price Range</h4>
