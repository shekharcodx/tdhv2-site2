...
export default function CarCard({
  ...
  >("daily");
  const [imgSrc, setImgSrc] = useState(image || "/assets/car_placeholder.jpg");

  const getPriceForPeriod = () => {
    switch (selectedPeriod) {
      case "daily":
        return { amount: dailyPrice, label: "/day" };
      case "weekly":
        return { amount: weeklyPrice, label: "/week" };
      case "monthly":
        return { amount: monthlyPrice, label: "/month" };
    }
  };
  ...
  return (
    <div className="flex-shrink-0 w-full sm:w-[380px] bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 ease-out-cubic group border border-soft-grey/30">
      <Link href={`/car/${id}`} className="block">
        <div className="relative h-56 overflow-hidden bg-gradient-to-br from-slate-teal/5 to-transparent">
          <Image
            src={imgSrc}
            alt={`${brand} ${name}`}
      ...
      <div className="p-6">
        ...
        <div className="space-y-2.5 mb-5">
          <div className="flex items-center gap-2.5 text-sm text-grey">
            ...
            <span className="font-medium">Minimum 1 day</span>
          </div>
          <div className="flex items-center gap-2.5 text-sm text-grey">
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-2">
            <button className="flex-1 flex items-center justify-center gap-2 bg-white border-2 border-slate-teal/30 text-slate-teal hover:border-slate-teal hover:bg-slate-teal/5 px-4 py-2.5 rounded-xl font-semibold transition-all duration-300 text-sm">
              <Phone className="w-4 h-4" />
              Call
            </button>
            <button className="flex-1 flex items-center justify-center gap-2 bg-white border-2 border-site-accent/30 text-site-accent hover:bg-site-accent hover:text-white hover:border-site-accent px-4 py-2.5 rounded-xl font-semibold transition-all duration-300 text-sm hover:shadow-lg">
